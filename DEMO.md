...
**Phase 11（暫停 — 即將推出）：機密會議走非美國雲 LLM**

> spec / schema / UI / worker 路徑都已實作完成，目前 UI 上 strict 選項被 `STRICT_AVAILABLE = false` flag 鎖住，等 Together AI 預算到位 + 確認 zh-TW 適配模型（候選 Qwen 2.5 72B / DeepSeek-V3）再 flip flag 開放。
