...
- **嚴格隱私模式**（即將推出）：機密會議走非美國雲 LLM；spec / schema / worker 路徑已實作，等模型選型 + 預算到位再開放
...
Groq Whisper-large-v3 STT (免費 tier) · pyannote.audio 3.3.2 diarization · Resemblyzer 聲紋 · Anthropic Claude Sonnet 4.5 via OpenRouter · OpenAI text-embedding-3-small（議題聚類） · _嚴格隱私模式即將推出 — 詳見下方_
...
| 嚴格隱私模式（即將推出） | Together / DeepSeek / Qwen 候選中 | 機密會議專屬路線，避免資料進美國雲。schema + worker 路徑已實作（`meetings.llm_provider`），等模型選型 + 預算到位 |
...
更完整的功能 demo（聲紋註冊、議題時間軸、影響圈）見 [DEMO.md](./DEMO.md)。
